...
import com.salespilot.api.application.exception.NoMeetingsFoundException;
...
public class GetAllMeetingsUseCase {
    ...
    public MeetingPageResponseDTO execute(String title, String clientCompanyName, UUID collaboratorId, Pageable pageable) {
        return new MeetingPageResponseDTO(
                ...
                repository.getAllMeetings(title, clientCompanyName, collaboratorId, pageable)
                        .map(m -> {
                ...
                new SummaryResponseDTO(
                    ...
                    .orElseThrow(() -> new NoMeetingsFoundException())
                )
                );
